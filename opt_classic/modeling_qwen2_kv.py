# Source: adapted from modeling_llama_kv.py for Qwen2.5 family
...
"""PyTorch Qwen2 model with KV-cache support for speculative decoding."""
import math
...
try:
    ...
    from transformers import AutoConfig as Qwen2Config

logger = logging.get_logger(__name__)


# ---------------------------------------------------------------------------
# Mask utilities (identical to modeling_llama_kv.py)
# ---------------------------------------------------------------------------
...
class Qwen2RotaryEmbedding(nn.Module):
    """Standard RoPE — reads rope_theta from config."""
...
# ---------------------------------------------------------------------------
# MLP  (Qwen2.5: SwiGLU, no MLP bias)
# ---------------------------------------------------------------------------
...
class Qwen2Attention(nn.Module):
    """
    Multi-headed attention for Qwen2.5.

    Key difference from LLaMA: Q/K/V projections use bias (config.attention_bias, default True).
    O projection has no bias.
    rope_theta is read from config (default 1_000_000 for Qwen2.5).
    ...
    def __init__(self, config: Qwen2Config):
        ...
        # [MODIFIED] Qwen2.5: Q/K/V have bias (attention_bias=True by default), O has no bias
    ...
    def _init_rope(self):
        # [MODIFIED] Read rope_theta from config (Qwen2.5 default: 1_000_000)
    ...
    def forward(
        ...
        # [MODIFIED] KVCache mechanism (same as modeling_llama_kv.py)
        if past_key_value is not None:
            if is_draft:
        ...
        # Qwen2.5 Q/K values are much larger than LLaMA, causing fp16 dot product overflow.
...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    def forward(
        ...
        # [MODIFIED] tree attention mask handling (same logic as modeling_llama_kv.py)
