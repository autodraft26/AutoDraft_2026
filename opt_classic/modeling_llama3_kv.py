...
def _make_causal_mask(
    ...
    # Ensure tensor creation dtype matches the finfo source dtype to avoid
    # overflow when global default dtype is fp16.
...
class LlamaLlama3RotaryEmbedding(nn.Module):
    """
    Llama 3.x per-frequency RoPE scaling.
    Uses rope_scaling fields:
      - factor
      - low_freq_factor
      - high_freq_factor
    ...
    def __init__(self, dim, config, max_position_embeddings=2048, base=10000, device=None):
        ...
        # [MODIFIED] transformers >=5.x moves rope_theta inside rope_scaling dict
        rope_theta = getattr(config, "rope_theta", None)
        if rope_theta is None:
            rs_tmp = getattr(config, "rope_scaling", None) or {}
            rope_theta = rs_tmp.get("rope_theta", base)
        self.base = rope_theta

        rs = config.rope_scaling
        if rs is None or not isinstance(rs, dict):
            raise ValueError(f"rope_scaling must be a dict for llama3. got: {rs}")

        factor = float(rs["factor"])
        low_freq_factor = float(rs["low_freq_factor"])
        high_freq_factor = float(rs["high_freq_factor"])
        old_context_len = float(rs["original_max_position_embeddings"])
        ...
        # 2) llama3 per-frequency scaling ( + )
        low_freq_wavelen = old_context_len / low_freq_factor
        high_freq_wavelen = old_context_len / high_freq_factor

        wavelen = 2 * math.pi / inv_freq  # wavelength

        # low freq  ->
        inv_freq_scaled = torch.where(wavelen > low_freq_wavelen, inv_freq / factor, inv_freq)

        # mid band smooth interpolation
        smooth = (old_context_len / wavelen - low_freq_factor) / (high_freq_factor - low_freq_factor)
        smooth = torch.clamp(smooth, 0.0, 1.0)

        inv_freq_smooth = (1.0 - smooth) * (inv_freq_scaled / factor) + smooth * inv_freq_scaled

        is_medium = (wavelen <= low_freq_wavelen) & (wavelen >= high_freq_wavelen)
        inv_freq_final = torch.where(is_medium, inv_freq_smooth, inv_freq_scaled)
...
class LlamaAttention(nn.Module):
    ...
    def _init_rope(self):
        ...
        if not isinstance(rs, dict):
            raise ValueError(f"Unexpected rope_scaling type: {type(rs)} -> {rs}")
        ...
        if scaling_type in ("linear",):
        ...
        elif scaling_type in ("dynamic", "ntk"):
            self.rotary_emb = LlamaDynamicNTKScalingRotaryEmbedding(
                self.head_dim,
                max_position_embeddings=self.max_position_embeddings,
                scaling_factor=float(scaling_factor),
            )
        elif scaling_type in ("llama3",):
            self.rotary_emb = LlamaLlama3RotaryEmbedding(
                self.head_dim,
                config=self.config,
                max_position_embeddings=self.max_position_embeddings,
                device=None,
            ...
            raise ValueError(f"Unknown RoPE scaling type: {scaling_type} (rope_scaling={rs})")
    ...
    def forward(
        ...
        if tree_attention_mask is None:
            ...
            if is_draft:
                ...
                # Use model compute dtype for mask fill to avoid fp16 overflow.
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    # transformers>=5.x tied weight dict
    ...
    def _reinit_rope(self):
        for layer in self.model.layers:
            ...
            old_device = attn.rotary_emb.inv_freq.device
            # Recreate RoPE with correct inv_freq (handles LlamaLlama3RotaryEmbedding etc.)
            attn._init_rope()
            # Move buffers to the correct device
            attn.rotary_emb = attn.rotary_emb.to(old_device)
